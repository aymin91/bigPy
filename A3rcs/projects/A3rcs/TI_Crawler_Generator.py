...
folder_path = '/'.join(rep_dir.split('/')[:-1]).format(yyyy=dates[0:4], blog_code=BLOG_CODE,
                                                       yyyymmdd=dates)


id_list = pd.read_csv(id_path+'TI_ID_List.csv', engine='python', encoding = 'utf-8') # , encoding = 'utf-8'
...
def generate_ti_crawler(user_info, start_date, end_date):

    try :
        start_time = time.time()

        full_path = rep_dir.format(yyyy=dates[0:4], blog_code=BLOG_CODE, yyyymmdd=dates,
        ...
        all_content_info = ti_crawler.gather_all_content(blog_url, start_date, end_date,
                                                         newest_post_num = user_info['newest_post_num'],
                                                         oldest_post_num = user_info['oldest_post_num'])

        ti_crawler.make_blog_DF(all_content_info, user_info['user_id'], full_path)
        ...
        user_info['collecting_date'] = collecting_day
        user_info['oldest_post_num'] = user_info['newest_post_num']
        # print(user_info)
        is_success = True
...
def concat_post_date(folder_path) :
    ...
    try :
        ...
        pblog_date_path = root_dir + util.get_pblog_date_dir(file_dir).format(yyyy=dates[0:4], blog_code=BLOG_CODE,
                                                                              yyyymmdd=dates)
...
def main(id_list):
    ...
    for user_info in id_list:
        ...
        if is_valid:
            ...
            try:
                ...
                crawler_success = generate_ti_crawler(user_info, start_date, end_date)
                new_id_list.append(user_info)
    ...
    # new_id_list_df = pd.DataFrame.from_dict(new_id_list, columns= cols)

    cols = ['user_id', 'user_nick', 'collecting_date', 'oldest_post_num', 'newest_post_num']

    new_id_list_df = pd.DataFrame.from_dict(new_id_list)
    new_id_list_df = new_id_list_df[cols]

    new_id_list_df.to_csv(id_path+'{blog_code}_ID_List.csv'.format(blog_code=BLOG_CODE), index=False, encoding='utf-8')
    # PermissionError: [Errno 13] Permission denied: 'C:/Projects/stg_server/repository/user_id/TI_ID_List.csv'   한번 수집후 두번 돌릴때마다 에러 : 파일을 열고 있었음
