from module.s1_collect.blog.bl_nv import nv_blog_post_crawler as nv_crawler
...
valid           = util.get_valid(BLOG_CODE)
dates           = util.get_today_day()
# collecting_day  =  str(int(dates) - 1)
collecting_day  =  str(int(dates) - 2)
...
id_list = pd.read_csv(id_path+'{blog_code}_ID_List.csv'.format(blog_code=BLOG_CODE), encoding = 'utf-8')
...
def generate_nv_crawler(user_info, start_date, end_date):
    try:
        ...
        driver           = nv_crawler.get_blog_url(user_info['user_id'])
        board_url_list   = nv_crawler.get_all_board_url(driver)
        all_content_info = nv_crawler.gather_all_content(board_url_list, start_date,end_date)

        nv_crawler.make_blog_DF(all_content_info, user_info['user_id'], full_path)
        ...
        user_info['crawling_date'] = collecting_day
    ...
    if is_valid :
        for user_info in id_list:
            ...
            try:
                ...
                crawler_success = generate_nv_crawler(user_info, start_date, end_date)
