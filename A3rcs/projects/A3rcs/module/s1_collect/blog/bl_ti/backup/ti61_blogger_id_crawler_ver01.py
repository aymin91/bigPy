...
def get_blogger_info(blog_info):
    ...
    blogger_id.tistory.comm 형태의 주소 blogger_id, blogger_nick을
    dict 형태로 수집
        :param     blog_info(bs4)
...
def gather_blogger_info(driver):
    ...
    for cat in categorys:
        ...
        open_page = scr_to_end(driver)  # driver.가 아니라 web_page를 돌렸을 때 드라이버가 없다는 오류가 났었음
        time.sleep(0.5)
...
def make_save_df(all_blogger_info):
    ...
    blogger_info_df.to_csv(
        'TI_blogger_id_{0}_{1}_{2}_{3}_{4}.csv'.format(now.year, now.month, now.day, now.hour, now.minute), index=False,
