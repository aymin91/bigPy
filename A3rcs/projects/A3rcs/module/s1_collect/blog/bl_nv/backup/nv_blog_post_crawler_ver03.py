...
#           - driver.find_element_by_xpath --> driver.find_element_css_selector (get_last_page_num, get_board_url)
#
#       2019.11.11
...
#           - def get_board_url in 'board_url' variable name is changed to 'b_url' by MIN
#
#       2019.11.15
...
#
#       2019.11.18
#           - the name of 'def board_url_collect' is changed to 'def get_board_url' by MAY
#
#
#
...
def get_all_board_url(driver) :
    ...
    get_board_url 모듈을 사용, 전체 글 url을 수집해 리스트로 반환합니다.
        :param   driver(ChromeWebDriver)
    ...
    for i in range(last_page_num) :
        ...
        board_url = get_board_url(driver)
...
def nick_present(content_info, blog_div, start_date, end_date) :
    '''
    ...
    nick_in_info = dict()
    upload_date = util.regex_convert_date(content_info.find('span', class_='se_publishDate pcol2').text)  # 날짜
...
def nick_no_present(content_info, blog_div, start_date, end_date) :
    '''
    parsing된 데이터인 all_content와 닉네임 여부인 blog_div를 받아,
    ...
    upload_date = util.regex_convert_date(upload_date = content_info.find('p', class_='date fil5 pcol2 _postAddDate').text)  # 날짜
...
def find_content_info(board_url, start_date, end_date) :  # 신형 파서
    ...
    정보를 반환합니다.
        :param    board_url(str), start_date(str), end_date(str)
    ...
    if is_vaild :
        content_info = nick_present(board_content_info, 'IN_INFO', start_date, end_date)
    else :
        content_info = nick_no_present(board_content_info, 'NO_INFO', start_date, end_date)
...
def run_step(blogger_id, start_date, end_date):
    # step 1
    driver = get_blog_url(blogger_id)
    # step 2
    board_url_list = get_all_board_url(driver)
    # step 3
    all_content_info = gather_all_content(board_url_list, start_date,end_date)
    # step 4
    make_NV_DF(all_content_info, blogger_id)
    return