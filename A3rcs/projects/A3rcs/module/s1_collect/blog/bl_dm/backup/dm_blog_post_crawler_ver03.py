...
def get_board_content_info(m_board_url, board_url, start_date, end_date) :
    ...
    upload_date = util.regex_convert_date(brd_soup.find('time', {'class' : 'txt_time'}).text)
...
def gather_all_content(blogger_id, board_url_list, start_date, end_date):
    '''
       게시글의 모든 정보를 수집하고 합쳐주는 메소드
       :param : blogger_id(str), board_url_list(list), start_date(str), end_date(str)
    ...
    for b in board_url_list:
        m_board_url = util(BLOG_TYPE, b, blogger_id=blogger_id)
...
def run_step(blogger_id, start_date, end_date):
    '''
        dm_blog_post_crawler의 모든 모듈을 실행시켜주는 메소드
    ...
    # step 2
    board_url_list = get_all_board_url(blog_url)
    # step 3
    all_content_info = gather_all_content(blogger_id, board_url_list, start_date,end_date)
    # step 4
    make_DM_DF(all_content_info, blogger_id)
    return None