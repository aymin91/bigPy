from module.s1_collect.blog.bl_dm import dm_blog_post_crawler as dm_crawler
...
def generate_dm_crawler(user_info, start_date, end_date):
    ...
    try:
        ...
        full_path = rep_dir.format(yyyy=dates[:4], blog_code=BLOG_CODE, yyyymmdd=dates, user_id=user_info['user_id'])

        blog_url = dm_crawler.get_blog_url(user_info['user_id'])
        board_url_list = dm_crawler.get_all_board_url(blog_url)
        all_content_info = dm_crawler.gather_all_content(user_info['user_id'], board_url_list, start_date,
                                                         end_date)
        dm_crawler.make_blog_DF(all_content_info, user_info['user_id'], full_path)
    ...
    if is_valid :
        for user_info in id_list:
            ...
            try:
                ...
                crawler_success = generate_dm_crawler(user_info, start_date, end_date)
